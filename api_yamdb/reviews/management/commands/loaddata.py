...
TABLES_FOREIGN_KEYS = {
    'comments.csv': {'author': lambda pk: models.User.objects.get(pk=pk),
                     'review': lambda pk: models.Review.objects.get(pk=pk)},
    'genre_title.csv': {'genre': lambda pk: models.GenreTitle.objects.get(genre_id=pk),
                        'title': lambda pk: models.GenreTitle.objects.get(title_id=pk)},
    'review.csv': {'author': lambda pk: models.User.objects.get(pk=pk),
                   'title': lambda pk: models.Title.objects.get(pk=pk)},
    'titles.csv': {'category': lambda pk: models.Category.objects.get(pk=pk)},
}
        ...
        for file in files:
            try:
                with open('/'.join([CSV_DATA_PATH, file]),
                    ...
                    # Формируем данные.
                    # Если поле является ForeignKey, то берем из связанной модели конкретный экземпляр
                    # с помощью TABLES_FOREIGN_KEYS
                    for row in reader:
                        data = {}
                        for k, v in row.items():
                            if TABLES_FOREIGN_KEYS.get(file) and k in TABLES_FOREIGN_KEYS.get(file):
                ...
                self.stdout.write(self.style.SUCCESS(f'...Данные успешно загружены из файла {file}...'))
